...
* 实例构造器和类(引用类型)
  ...
  * 构造引用类型的对象时,在调用类型的实例构造器之前,为对象分配的内存总是先被 **归零**,没有被构造器显式初始化的所有字段都保证获得0或null值(VS验证 被初始化为null后可以通过编译,但是运行时有可能出现 **空指针异常**,但编译的时候没有"**使用未初始化的局部变量**"编译错误)
...
* 泛型方法
  * 在定义泛型类、结构、接口时(三者本质都会被编译成类型),类型中定义的任何方法(任何局部)都可以引用类型指定的类型参数(还未指定).即类型参数可以作为 **方法参数**,**方法返回值**,**方法内部定义的局部变量**.因为在类型层面上的类型参数的作用域是整个类.
  * 然而CLR还与允许方法指定它自己的类型参数,即 **泛型方法**.这些类型参数也可以作为参数,返回值或局部变量的类型使用.泛型方法的存在为开发人员提供了极大的便利性
  * 泛型方法和类型推断
    * C#的泛型语法涉及大量<和>符号,为了改进代码的创建,增强可读性和可为维护性,C#编译器支持在调用泛型方法时进行 **类型推断**.这意味着编译器会在调用泛型方法时根据方法的参数类型和返回值类型自动判断要使用的类型(如果可以依靠它们推断出来的话或者存在对应的重载版本),以便 **省略**<泛型参数>内容.
    * 类型可以定义多个方法,让其中一个方法接受具体数据类型,让另一个方法接受类型参数(即泛型可以作为签名的一种进行 **重载** ).但是C#编译器优先考虑较明确的匹配,再考虑泛型匹配,即非泛型方法优先匹配于类型推断的泛型方法.但是如果明确指定了泛型类型实参,这会告诉编译器不推断类型实参,这时候编译器会毫不犹豫的调用泛型版本的方法.
* 泛型和其它成员
  * 在C#中,属性、索引器、事件、操作符、构造器和终结器本身不能有类型参数，但是它们能在泛型类中定义,这些成员中的代码也能使用类型的类型参数(即使用 **全局** 的类型参数)
  * C#不允许这些成员指定自己的泛型参数是因为Microsoft C#团队认为开发人员很少需要将这些成员作为泛型使用,而且为这些成员添加泛型支持的代价很高,需要设计足够的语法.
* 可验证性和约束
  * 解释泛型代码时,C#编译器会进行分析,确保代码适用于当前已有或将来可能定义的任何类型.从表面看,使用泛型似乎做不了太多事情.只能声明泛型类型的变量，执行变量赋值,再调用Object的方法,仅此而已!显然如果泛型只能这么用可以说泛型几乎没有任何作用.幸好,编译器和CLR支持称为 **约束** 的机制, **通过约束可以使泛型变得真正有用!**
  * 约束的作用是限制能指定成泛型实参的类型变量，通过限制类型参数，可以对那些类型执行更多操作(不再仅仅是调用Object的方法).C#的 **where** 关键字告诉编译器,为T指定的任何类型实参都必须实现主要约束和次要约束.现在,当代码引用泛型类型或方法时,编译器要负责保证类型实参符合指定的约束.(**编译时类型安全**).
  * **注意** 的几点:
    * 约束可以应用于泛型类型的类型参数,也可应用于泛型方法的类型参数.
    * CLR不允许基于类型参数名称或约束进行重载,只能基于元数(类型参数个数)对类型或方法进行重载
    * 重写虚泛型方法时,重写的方法必须指定相同数量的类型参数,而这些类型参数会继承在基类方法上指定的约束,C#编译器不允许为重写方法的类型参数指定任何约束.(也就是只有最基类型可以指定约束,它的任何子类都会继承这个约束).
  * 主要约束:
    * 类型参数可以指定 ***零个或一个*** 主要约束. **主要约束** 是可以代表非密封类的一个引用类型(描述得并不准确).指定主要约束后相当于对编译器承诺:一个指定的类型实参必须是类型约束的类型或者 **其派生类型**.
    * 主要约束不能指定一下特殊的引用类型:System.Object,System.Array,System.Delegate,System.ValueType,System.Enum,System.Void.如果类型参数没有指定主要约束,就默认为System.Object(那不就相当于没指定了...)
    * 有两个特殊的主要约束: **class** 和 **struct** .其中,class约束向编译器承诺实参是引用类型,struct向编译器承诺是值类型(包括枚举在内的任何值类型都满足这个约束.)
  * 次要约束:
    * 类型参数可以指定 ***零个或多个*** 次要约束,**次要约束** 代表 **接口类型**.这种约束向编译器承诺类型实参实现了接口,由于能指定多个接口约束,所以类型实参必须实现了所有接口约束(以及主要约,如果有的话)
  * 构造器约束:
    * 类型参数可以指定 ***零个或一个*** 构造器约束, **构造器约束** 向编译器承诺类型实参时实现了 **公共无参构造器** 的非抽象类型.这是最简单的也是最后一种约束.使用 **new()** 进行约束  
    `internal sealed class Constructor<T> where T: new() `  
    另外,所有的值类型是都隐式提供公共无参构造器的,但是有的引用类型不提供无参构造器
  * 泛型变量的转型.
    * 直接将泛型变量转型为其它类型是非法的,除非转型为与约束兼容的类型.强制转型的情况下建议使用 **as** 操作符.
  * 将泛型变量初始化(设为默认值)
    * 直接将泛型类型变量设为null是非法的(无法通过编译),除非将泛型类型约束为引用类型.
    * 如果未对T进行约束,那么它 **可能** 是值类型,而将值类型的变量是不能为null的.C#团队认为有必要允许开发人员将变量设为它的默认值,并为此提供了 **default** 关键字.   
    `T temp = default(T);`  
    default关键字告诉CLR的JIT编译器,如果T是引用类型,就将temp设为null,如果T是值类型,就将其所有位设为0.
  * 将泛型类型变量与null进行比较.  
    如果泛型变量没有被约束,那么使用==或!=操作符将泛型变量与null进行比较是合法的.如果T是值类型,那么比较结果永远是不等.如果泛型变量被约束为struct,编译器才会报错:值类型的变量不能与null进行比较.
  * 两个泛型类型变量相互比较  
    * 如果泛型类型参数不能肯定是引用类型,对同一个泛型类型的两个变量进行比较是非法的.两个值类型的变量相互比较是非法的,除非值类型重载了==和!=操作符(FCL提供的基元值类型允许比较是因为C#编译器知道如何生成正确的代码,然而对于非基元值类型,C#编译器不知道如何生成代码来比较).而引用类型的比较是合法的(我认为==操作符应该是调用了 **Equals方法**)
    * C#编译器不允许将类型参数约束成具体的值类型,因为值类型 **隐式密封**,不可能存在从值类型派生的类型.若果允许将类型参数约束成具体的值类型,那么泛型方法会被约束成只支持该值类型,还不如不要泛型直接用该值类型了.
    * 泛型类型变量作为操作数使用
      * 最后要注意,将操作符应用于泛型的操作数会出现大量问题.因为C#编译器知道如何解释应用于基元类型的操作符(如+ - * / 等),但是不能将这些操作符应用于泛型类型的变量.
...
* 关于调用接口方法的更多探讨
  ...
  * 注意:接口本身是 **不能被实例化** 的.上述的的"接口实例化"可以这样理解，实现接口的类是该接口的一个子类，可以将子类的实例赋值给其父类，也即接口，就像抽象类不能被实例化而实现其全部方法的子类可以被实例化并被赋值给它一样
* 隐式和显式接口方法实现(的内部细节)
