...
---
## 数据表之间的三种关系
SQL Server 2008是关系数据库管理系统,当数据库中包含多个表时,需要通过 **主关键字** 或者 **唯一约束列** 以 **作为外键** 的形式在另一个表中存在,从而建立该表和另一个表的关系,使各表之间能够协调工作. **关系** 是通过 **匹配键列中的数据** 工作的,而键列通常是两个表中具有相同名称的列(当需要一对一相同时,这样建立一对一联系).表和表之间存在如下三种关系:
#### 一对一关系  
一对一关系是指表A中的一条记录在表B中 **有且只有一条** 相匹配的记录.表B中的一条记录在表A中也 **有且只有一条** 匹配行. 这就要求两个键列 **均为主键或唯一约束列**,其中一个设为外键 在一对一关系中,大部分相关信息都在一个表中.一对一关系主要适用于:
  * 分割一个含有许多列的表
  * 处于安全考虑而隔离表的某一部分(保护数据的思想)
  * 存储容易删除的临时数据,因为只需删除表即可删除这些数据
  * 存储只应用于主表子集的信息(黑人问号??)  

#### 一对多关系
一对多关系是指表A中的一条记录在表B中 **有零或多条** 匹配行,但是表B在表A中 **有且只有一条** 匹配行. 这就要求两个相关键列中 **只有一列为主键或唯一约束列**.

#### 多对多关系
多对多关系是指关系中的每个表的行在相关表中具有多个匹配行,在SQL Server中,多对多关系是依靠第3个表(即连接表)实现的,连接表包含相关的两个表的主键列或唯一约束列,然后从两个相关表的主键列分为创建与联机表中匹配列的关系,也就是多对多关系 **相当于两个一对多关系**
